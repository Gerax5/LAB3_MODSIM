...
def bfgs(f, df, x0, alpha, tol=1e-6, maxIter=1000, prt=0, stop="grad_norm"):
    ...
    for _ in range(maxIter):
        ...
        x = x + alpha * p
        fx = f(x); g = df(x)
        s = x - x_prev; y = g - g_prev; ys = float(np.dot(y, s))
        if ys > 1e-12:
            rho = 1.0/ys; I = np.eye(n); V = I - rho*np.outer(s, y); H = V @ H @ V.T + rho*np.outer(s, s)
        xs.append(x.copy()); fxs.append(fx); iters += 1
        err = compute_error(stop, x_prev, x, f_prev, fx, g); errs.append(err)
        if prt: print(f"[bfgs] iter={iters}  f={fx:.6e}  ||g||={norm(g):.3e}")
